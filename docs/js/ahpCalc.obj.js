 ...
 */
ahpCalc.consistencyIndex = [0, 0, 0.58, 0.9, 1.12, 1.24, 1.32, 1.41, 1.45, 1.49, 1.51, 1.48, 1.56, 1.57, 1.58 ];
/**
...
ahpCalc.calculateResults = function(resultArray){
	...
	return {resultColumn: _resultColumn, consistencyRatio: _consistencyRatio.cr,consistencyIndex:_consistencyRatio.ci, eigen: _consistencyRatio.eigen};
};
...
ahpCalc._calcConsistencyRatio = function(resultArray, resultColumn){
	...
	if (this.consistencyIndex[options-1]!=0){
		var cr = ci / this.consistencyIndex[options-1];
	}else{
		var cr = 0;
	}

	return {cr: cr, eigen: eigen, ci:ci};
};
